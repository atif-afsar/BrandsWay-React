...
const ServiceCard = ({ service, index }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -8 }}
      className="relative shrink-0 w-[300px] md:w-[340px] h-[460px] rounded-3xl flex flex-col justify-between group cursor-pointer overflow-hidden"
      style={{
        background: 'white',
    ...
    >
      ...
      <div
        className="absolute top-6 right-7 text-[72px] font-black leading-none select-none z-10 transition-colors duration-300 group-hover:text-white/10"
      ...
      <div className="relative z-10 p-8 flex flex-col h-full justify-between">
        <div>
          {/* Icon */}
          <div
            className="w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-400 group-hover:bg-white/15"
          ...
          {/* Title */}
          <h3
            className="mt-8 text-[22px] font-bold tracking-tight leading-tight transition-colors duration-300 group-hover:text-white"
          ...
          {/* Divider */}
          <div
            className="mt-4 h-px w-10 transition-all duration-300 group-hover:w-16 group-hover:bg-white/40"
            style={{ background: '#C61407' }}
          />

          {/* Description */}
          <p
            className="mt-5 text-sm leading-relaxed transition-colors duration-300 group-hover:text-white/75"
            style={{ color: '#6b7280' }}
          >
            {service.desc}
          </p>
...
const ServicesSection = () => {
  ...
  // Translate from 0 to -(SCROLL_DISTANCE)px in px units
  ...
  // Counter: which card are we on roughly
  const cardIndex = useTransform(scrollYProgress, [0.05, 0.95], [1, TOTAL_CARDS]);
      ...
      >
        <div className="sticky top-0 flex h-screen flex-col justify-center overflow-hidden">
          ...
          <div
            ...
            style={{
              background: 'radial-gradient(circle, rgba(198,20,7,0.04) 0%, transparent 70%)',
            }}
          />
          ...
          <div className="flex flex-col gap-10 w-full">
            ...
            >
              ...
              <h2
                className="text-4xl md:text-[clamp(48px,7vw,80px)] lg:text-4xl font-black text-black tracking-tighter leading-none mt-6"
                style={{ fontFamily: '"Sora", sans-serif' }}
              >
                Marketing  
                <span className='font-medium' style={{ color: '#C61407' }}>
                  Services
                </span>
              </h2>
              ...
              <motion.p
                ...
                style={{ fontFamily: '"Sora", sans-serif' }}
              >
                <motion.span style={{ color: '#C61407' }}>
                  {/* We'll show it textually via transform */}
                </motion.span>
                  ...
                  <motion.span
                    ...
                    style={{
                      opacity: useTransform(
                        scrollYProgress,
                        [
                          0.05 + (i / TOTAL_CARDS) * 0.85,
                          0.05 + ((i + 1) / TOTAL_CARDS) * 0.85,
                        ],
                        [1, 0.25]
                      ),
