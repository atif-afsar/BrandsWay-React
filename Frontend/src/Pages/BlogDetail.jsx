...
import { SITE_ORIGIN } from "../seo/business.js";
import {
  organizationSchema,
  websiteSchema,
  localBusinessSchema,
...
const BlogDetail = () => {
  ...
  const graphLd = useMemo(() => {
    if (!blog) return null;
    const absImage = blog.image?.startsWith("http") ? blog.image : `${SITE_ORIGIN}${blog.image}`;
    const faqs = blog.content?.faq || [];
    const graph = [
      organizationSchema(),
      websiteSchema(),
      localBusinessSchema(),
      articleSchema({
        ...
        url: canonicalUrl,
        image: absImage,
        datePublished: blog.datePublished || "2026-01-15",
        dateModified: blog.dateModified || blog.datePublished || "2026-01-15",
      }),
      breadcrumbSchema([
        { name: "Home", url: "/" },
    ...
    if (faqs.length) graph.push(faqPageSchema(faqs));
    return { "@context": "https://schema.org", "@graph": graph };
  }, [blog, canonicalUrl, canonicalPath, slug]);

  if (!blog) {
    return (
      <div className="bg-[#080C12] min-h-screen flex items-center justify-center px-4">
        <SEOHead
          title="Insight Not Found | The BrandsWay"
          description="This article may have moved. Browse insights from The BrandsWay, a digital marketing and PR agency in Aligarh."
        ...
        <div className="text-center">
          ...
          <button
            ...
            className="mt-6 px-6 py-3 bg-[#C61407] text-white rounded-full font-bold hover:bg-red-700 transition-all"
          >
            Return to Journal
          </button>
        </div>
  ...
  const pageTitle = blog.metaTitle || `${blog.title} | The BrandsWay Insights`;
  ...
  return (
    <div className="bg-[#FAF9F6] min-h-screen selection:bg-[#C61407] selection:text-white overflow-x-hidden">
      <SEOHead
        ...
        keywords={`${blog.category}, digital marketing Aligarh, PR agency Aligarh, The BrandsWay`}
      ...
      <main className="pt-32 pb-20">
        <nav className="max-w-4xl mx-auto px-6 mb-8" aria-label="Breadcrumb">
          <ol className="flex flex-wrap items-center gap-2 text-xs text-gray-500">
            <li>
              <Link to="/" className="hover:text-[#C61407]">
                Home
              </Link>
            </li>
            <li aria-hidden="true">/</li>
            <li>
              <Link to="/insights" className="hover:text-[#C61407]">
                Insights
              </Link>
            </li>
            <li aria-hidden="true">/</li>
        ...
        <header className="max-w-4xl mx-auto px-6 mb-12 sm:mb-20 text-center sm:text-left">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold text-[#080C12] tracking-tighter leading-[1.05] mb-8">{blog.title}</h1>
            ...
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-6 py-6 border-y border-gray-200/60 text-gray-500 font-medium text-xs sm:text-sm">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gray-200 overflow-hidden border border-gray-100">
                  <img
                    src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(blog.author)}`}
                    alt=""
                    loading="lazy"
                    decoding="async"
                    width={40}
              ...
              <div className="flex items-center gap-2">
                <Clock size={16} className="text-[#C61407]" aria-hidden />
        ...
        <article className="max-w-3xl mx-auto px-6 relative">
          <div className="mb-12">
            <p className="text-xl sm:text-2xl text-gray-800 leading-relaxed font-serif italic">{blog.content.intro}</p>
          </div>
          ...
          <div className="prose prose-lg prose-red max-w-none">
            {blog.content.sections.map((section, index) => (
              <motion.div
                ...
                className="mb-16"
              >
                <h2 className="text-3xl sm:text-4xl font-bold text-[#080C12] mb-6 tracking-tight">{section.title}</h2>
                <div className="text-gray-600 leading-[1.8] text-lg font-light whitespace-pre-line">{section.content}</div>
              </motion.div>
            ))}
          </div>
        </article>
        ...
        {blog.content.faq && blog.content.faq.length > 0 && (
          <section className="max-w-3xl mx-auto px-6 mt-20 mb-20" aria-labelledby="blog-faq-heading">
            <h2 id="blog-faq-heading" className="text-2xl font-bold text-[#080C12] mb-10 flex items-center gap-4">
              ...
              {blog.content.faq.map((faq, index) => (
                <div
                  ...
                  className={`border transition-all duration-300 rounded-2xl ${
                    expandedFaq === index ? "border-[#C61407] bg-white shadow-xl shadow-red-900/5" : "border-gray-200 bg-gray-50/50"
                  }`}
                >
                  <button
                    type="button"
                    onClick={() => setExpandedFaq(expandedFaq === index ? null : index)}
                    className="w-full px-6 py-5 flex items-center justify-between text-left gap-3"
                    aria-expanded={expandedFaq === index}
                    aria-controls={`blog-faq-${index}`}
                    id={`blog-faq-btn-${index}`}
                  >
                    <span className="font-bold text-gray-900">{faq.question}</span>
                    <ChevronDown
                      size={20}
                      className={`text-[#C61407] shrink-0 transition-transform duration-300 ${expandedFaq === index ? "rotate-180" : ""}`}
                      aria-hidden
                    />
                  </button>
                  {expandedFaq === index && (
                    <motion.div
                      id={`blog-faq-${index}`}
                      role="region"
                      aria-labelledby={`blog-faq-btn-${index}`}
        ...
        <section className="max-w-3xl mx-auto px-6 mt-12" aria-labelledby="internal-services-heading">
          <h2 id="internal-services-heading" className="sr-only">
            Related marketing services in Aligarh
          </h2>
          <p className="text-sm text-gray-500 leading-relaxed">
            Planning execution next? Explore our{" "}
            <Link to="/seo-company-aligarh" className="text-[#C61407] font-semibold hover:underline">
              SEO company in Aligarh
            </Link>
            ,{" "}
            ...
            pages — each includes FAQs and conversion-focused detail.
          </p>
        </section>
          ...
          <div className="bg-[#080C12] rounded-[2rem] p-8 sm:p-16 relative overflow-hidden text-center sm:text-left">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10">
              <div className="lg:max-w-xl">
                ...
                <p className="text-gray-400 text-lg">{blog.content.cta}</p>
              </div>
              <Link to="/contact-us" className="relative z-10">
                <span className="inline-block px-10 py-5 bg-[#C61407] text-white font-black uppercase tracking-widest rounded-full hover:scale-105 active:scale-95 transition-all shadow-xl shadow-red-900/20 whitespace-nowrap">
                  Get Started
                </span>
              </Link>
            </div>
        ...
        <section className="max-w-7xl mx-auto px-6 py-20 border-t border-gray-200">
          <div className="flex items-center justify-between mb-12">
            ...
            <Link to="/insights" className="text-[#C61407] font-bold text-xs uppercase tracking-[0.2em] border-b-2 border-[#C61407] pb-1">
              View All
            </Link>
          </div>
              ...
              .map((post) => (
                <article key={post.id} className="group cursor-pointer">
                  <Link to={`/blog/${post.slug}`} className="block">
                    ...
                    <h3 className="text-xl font-bold group-hover:text-[#C61407] transition-colors mb-3">{post.title}</h3>
